...
public class AlphaVantageStockScrapperController : Controller
{
    ...
    [HttpPost("scrape")]
    public async Task<IEnumerable<Stock>> ScrapeStock([FromBody] StockScrapeQuery query, CancellationToken token)
    {
        while (true)
        {
            try
    ...
    [HttpPost("read")]
    public async Task<IEnumerable<Stock>> ReadStock([FromBody] StockScrapeQuery query, CancellationToken token)
    {
        using var client = InfluxDBClientFactory.Create(Constants.InfluxDBUrl, Constants.InfluxToken);
        ...
        return tables.SelectMany(table =>
            table.Records.Select(record => Stock.FromRecord(record)));
    }
